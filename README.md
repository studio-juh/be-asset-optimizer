...
AIモデル本体はGitに含めず、開発起動またはビルドの前に公式Real-ESRGAN Releaseから自動取得し、SHA-256を検証します。完成したインストーラーとポータブル版には従来どおりモデルが同梱されるため、利用時のダウンロードはありません。
...
モデルだけを事前取得する場合は `npm run prepare:ai` を実行します。一度取得すれば、モデルが更新または削除されるまで再ダウンロードしません。
